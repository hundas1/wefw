...
def _Market(base_tf: str, refresh: bool, source: str = "yahoo"):
    ...
    if source == "ibkr":
        from .feeds.ibkr import LoadIbkr1m

        m1 = LoadIbkr1m()
        if m1 is None:
            raise SystemExit("no IBKR history yet: run `rapier ibkr-backfill --start 2024-10-01` first")
        long_h1, rolls = D.RollAdjust(D.Fetch("1h", refresh=refresh))
        fr = D.FramesFrom1m(m1, long_h1)
        return Market.From1h(fr["1h"], {tf: fr[tf] for tf in lower}, base_tf=base_tf), rolls
    d = D.LoadNq(("1h",) + lower, refresh=refresh)
    return Market.From1h(d["1h"], {tf: d[tf] for tf in lower}, base_tf=base_tf), d["rolls"]
...
def CmdImport(a):
    for path in a.paths:
        added, rejected = D.ImportTape(path, a.interval)
        print(f"{path}: +{added} bars, {rejected} non-native day(s) rejected")


def CmdBacktest(a):
    from .report import Write
    from .system import Build, LoadParams

    params = LoadParams(a.config)
    if a.mode == "prop":
...
def CmdIbkrBackfill(a):
    _IbkrFeed(a).Connect().Backfill(a.start, a.end)


def CmdTradaraLogin(a):
    from .brokers import tradara as T

    verifier, challenge = T.PkcePair()
    print("1) Open this URL in the browser where you are logged in to Tradara and approve access:\n")
    print(T.AuthorizeUrl(challenge), "\n")
...
def CmdTrade(a):
    ...
    RunTrader(broker_kind=a.broker, feed=a.feed, armed=a.arm, once=a.once, config=a.config, cfg=cfg)
...
def Main(argv=None):
    ...
    IbArgs(bf, int(os.environ.get("RAPIER_IBKR_DATA_PORT", 4001)))
    bf.set_defaults(fn=CmdIbkrBackfill)

    tl = sub.add_parser("tradara-login", help="OAuth (PKCE) login; stores tokens in a local 600 file")
    tl.set_defaults(fn=CmdTradaraLogin)
    ...
    bc.set_defaults(fn=CmdBrokerCheck)
