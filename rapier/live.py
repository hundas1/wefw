...
in :mod:`rapier.trader` / :mod:`rapier.executor`, which reuse :func:`EngineView`.
"""
...
from .backtest import TF_DELTA, Market, _Asof, _ContextOk, Run
from .brokers.base import TickBracket
...
def BuildMarket(params: dict, refresh: bool = True) -> Market:
    ...
    return Market.From1h(d["1h"], frames, base_tf=min(lower, key=lambda x: int(x[:-1])) if lower else "1h")


def ArmedOrders(mkt: Market, params: dict, res=None) -> list[dict]:
    ...
    today = D.TradingDay(pd.DatetimeIndex([now]))[0]
    entered, day_pnl = {}, 0.0
    t = res.trades
    if len(t):
        et = pd.DatetimeIndex(pd.to_datetime(t.entry_time))
        entered = t[D.TradingDay(et) == today].groupby("book").size().to_dict()
        closed = t[t.exit_time.notna()]
        if len(closed):
            xt = pd.DatetimeIndex(pd.to_datetime(closed.exit_time))
            day_pnl = float(closed.pnl[D.TradingDay(xt) == today].sum())
    ...
    for b in books:
        ...
        for side, S in ((1, s.long), (-1, s.short)):
            ...
            if not _ContextOk(b, side, 0, arr(feat.pd_pos), arr(feat.vs_midnight), arr(feat.vs_dopen),
            ...
            if qty < 1:
                continue
            e_r, s_r, t_r = TickBracket(side, E, stop, b.tp1_r)
...
def EngineView(mkt: Market, params: dict, lookback_days: int = 20):
    ...
    if len(t):
        closed = t[t.exit_time.notna()]
        now_day = D.TradingDay(pd.DatetimeIndex([last]))[0]
        exits = pd.DatetimeIndex(pd.to_datetime(closed.exit_time))
        day_pnl = float(closed.pnl[(D.TradingDay(exits) == now_day)].sum()) if len(closed) else 0.0
    ...
    view = View(now=last + step, last_bar=last, last_close=float(base.close.iloc[-1]),
                armed=ArmedOrders(mkt, params, res), open_trades=list(res.open_trades),
...
def Cycle(params: dict, broker: EventSink, webhook: str | None, refresh: bool = True) -> list[dict]:
    mkt = BuildMarket(params, refresh=refresh)
    books, risk = Build(params)
    ...
    orders = {o["key"]: o for o in ArmedOrders(mkt, params, res)}
